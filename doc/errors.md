...
| <nobr>?MAP</nobr> | <nobr>Missing apostrophe</nobr> | Every conditional (opened with the " command) must be closed with the ' command. |
...
| <nobr>?TXT</nobr> | <nobr>Invalid text delimiter '*x*'</nobr> | Text delimiters must be graphic ASCII characters (decimal values 33 through 126, inclusive). |
